...
FROM(
  SELECT
 	...
 	FROM
	  immport.biosample, immport.expsample, immport.expsample_2_biosample, immport.expsample_2_file_info, immport.file_info, immport.arm_2_subject, immport.arm_or_cohort
 	WHERE
		...
		file_info.detail = 'Flow cytometry workspace') AS dataset
-- use IN filter to 'inherit' the global study/participant filter
WHERE study_accession IN (SELECT Label from study.StudyProperties)
